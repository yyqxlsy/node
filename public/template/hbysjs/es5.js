...
var _cookie = require("../libs/cookie.js");
		...
		banner: function banner(options) {
			...
			if (options.list != undefined && options.list.length > 0) {
				...
				this.LOCAL.listMove = function (i, type) {
					...
					}).end().eq(i).css({ left: options.items.eq(0).width() * type }).stop().animate({ left: 0 });
				};
		...
		value: function load() {
			...
			$.ajax({
				...
				success: function success(res) {
					switch (res) {
						case "0":
							$("#msg").html("用户名密码不符");break;
						case "1":
							$("#msg").html("请重新登录");break;
						default:
							that.res = JSON.parse(res);
							$("#msg").html("登录成功");
							setTimeout(function () {
				...
				success: function success(res) {
					switch (res) {
						case "0":
							$("#msg").html("重名");break;
						case "1":
							$("#msg").html("注册成功，3秒后跳转登录界面");
							setTimeout(function () {
								location.href = "login.html";
							}, 3000);break;
						case "2":
							$("#msg").html("不允许为空");break;
